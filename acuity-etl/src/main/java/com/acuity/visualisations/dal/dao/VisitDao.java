...
@Repository
public class VisitDao extends EntityDao<Visit> {
    private static final String RESULT_VISIT = "RESULT_VISIT";
    ...
    protected String getInsertStatement() {
        String tagetTable = RESULT_VISIT;
        List<TableField> fieldsToInsert = new ArrayList<TableField>();
        TableFieldBuilder fieldBuilder = new TableFieldBuilder(tagetTable);
        fieldsToInsert.add(fieldBuilder.setField("VIS_ID").build());
        fieldsToInsert.add(fieldBuilder.setField("VIS_UNQ_SHA1").build());
        fieldsToInsert.add(fieldBuilder.setField(getSecondaryHashColumnName()).build());
        fieldsToInsert.add(fieldBuilder.setField("VIS_REF_SHA1").build());
        fieldsToInsert.add(fieldBuilder.setField("VIS_DATE_CREATED").build());
        fieldsToInsert.add(fieldBuilder.setField("VIS_DATE_UPDATED").build());
        fieldsToInsert.add(fieldBuilder.setField("VIS_DATE").build());
        fieldsToInsert.add(fieldBuilder.setField("VIS_NUMBER").build());
        fieldsToInsert.add(fieldBuilder.setField("VIS_PAT_ID").build());
    ...
    protected String getUpdateStatement() {
        ...
        fieldsToSet.add(fieldBuilder.setField("VIS_DATE_UPDATED").build());
        fieldsToSet.add(fieldBuilder.setField("VIS_NUMBER").build());
        List<TableField> whereFields = new ArrayList<TableField>();
        whereFields.add(fieldBuilder.setField("VIS_ID").build());
        String sql = QueryBuilderUtil.buildUpdateQuery(tagetTable, fieldsToSet, whereFields);
        return sql;
    }

    @Override
    protected void prepareStatementToInsert(PreparedStatement ps, Visit entity) throws SQLException {
        int paramIndex = 1;
    ...
    @Override
    protected void prepareStatementToUpdate(PreparedStatement ps, Visit entity) throws SQLException {
        int paramIndex = 1;
        ps.setObject(paramIndex++, entity.getIntHashForSecondaryFields());
        ps.setTimestamp(paramIndex++, getSQLTimestamp(entity.getDateUpdated()));
        ps.setObject(paramIndex++, entity.getVisitNumber());
    ...
    public String getHashesStatement(Class<?> entityClass) {
        ...
        joinChain.add(builder.setSourceEntity(RESULT_VISIT).setTargetEntity("RESULT_PATIENT").addColumnToJoin("VIS_PAT_ID", "PAT_ID").build());
        ...
        hashColumnNames.add("VIS_UNQ_SHA1");
        hashColumnNames.add(getSecondaryHashColumnName());
        hashColumnNames.add(getIdColumnName());
        return QueryBuilderUtil.buildSelectHashesQuery(RESULT_VISIT, hashColumnNames, joinChain, fieldValues);
    }

    @Override
    public List<String> getSubjectsIdsByStudyName(String studyName) {
        return getJdbcTemplate().queryForList("SELECT DISTINCT VIS_PAT_ID FROM RESULT_VISIT e "
                + "left join RESULT_PATIENT p on (e.VIS_PAT_ID = p.PAT_ID) "
