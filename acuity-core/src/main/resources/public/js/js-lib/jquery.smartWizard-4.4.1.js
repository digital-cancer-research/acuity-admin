...
(function ($, window, document, undefined) {
  ...
  function SmartWizard(element, options) {
    ...
    if (this.options.navContainer) {
      this.nav = this.main.find(this.options.navContainer).children('ul'); // Step anchor elements
    } else {
    ...
    if (this.options.stepsContainer) {
      this.container = this.main.find(this.options.stepsContainer); // Content pages
    } else {
  ...
  $.extend(SmartWizard.prototype, {
    ...
    _setToolbar: function () {
      ...
      const nextButtonTemplate = this.options.toolbarSettings.nextButtonTemplate || "<button class='btn btn-secondary sw-btn-next' type='button'></button>";
      const previousButtonTemplate = this.options.toolbarSettings.previousButtonTemplate || "<button class='btn btn-secondary sw-btn-prev' type='button'></button>";

      var btnNext = this.options.toolbarSettings.showNextButton !== false ? $(nextButtonTemplate).text(this.options.lang.next) : null;
      var btnPrevious = this.options.toolbarSettings.showPreviousButton !== false ? $(previousButtonTemplate).text(this.options.lang.previous) : null;
      ...
      let addBtnGroupExtra = function(btnGroupExtra, toolbar, btnGroupMain, position) {
        if (!btnGroupExtra) {
          return;
        }
        ...
        switch (position) {
          case 'start':
            toolbar.prepend(btnGroupExtra);
            break;
          case 'inline':
            btnGroupMain.append(btnGroupExtra.children());
            break;
          default:
            toolbar.append(btnGroupExtra);
        }
      }

      const toolbarButtonPosition = this.options.toolbarSettings.toolbarButtonPosition;
      switch (this.options.toolbarSettings.toolbarPosition) {
        case 'top':
          ...
          addBtnGroupExtra(btnGroupExtra, toolbarTop, btnGroup, toolbarButtonPosition);
        ...
        case 'bottom':
          ...
          addBtnGroupExtra(btnGroupExtra, toolbarBottom, btnGroup, toolbarButtonPosition);
        ...
        case 'both':
          ...
          addBtnGroupExtra(btnGroupExtra.clone(true), toolbarBottom, btnGroup, toolbarButtonPosition);
    ...
    _loadStepContent: function (idx) {
      ...
      if (this.current_index !== null && this._triggerEvent("leaveStep", [curTab, this.current_index, idx, stepDirection]) === false) {
        return false;
      }
    ...
    stepState: function (stepArray, state) {
      ...
      if (selSteps && selSteps.length > 0) {
        switch (state) {
          ...
          case 'enable':
            $(selSteps).parents('li').removeClass('disabled').addClass("done");
            break;
          ...
          case 'initial':
            $(selSteps).parents('li').removeClass().addClass('nav-item');
            break;
        }
      }
    },
    currentStep: function() {       // backwards compatibility
